...
//returns the latency used for every command
...
static void *ftl_thread(void *arg)
{
    ...
    while (1) {
        for (i = 1; i <= n->nr_pollers; i++) {
            ...
            //Main returner of latency out of this whole thing, but should be adjustef
            req->reqlat = lat;
            req->expire_time += lat;
